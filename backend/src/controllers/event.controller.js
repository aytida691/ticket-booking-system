...
/**
 * Deletes an event, but only if NO booking — confirmed or cancelled —
 * has ever been made against any of its shows. This is stricter than
 * just checking confirmed bookings: a cancelled booking is still a real
 * historical/financial record (and its row still foreign-keys to the
 * show), so deleting the event out from under it would either violate
 * that constraint or silently destroy audit history. In practice this
 * means only events with zero booking activity can be hard-deleted;
 * anything with booking history should be handled by simply not
 * scheduling further shows for it, not by deletion.
 */
async function deleteEvent(req, res, next) {
  try {
    ...
    const anyBookings = await pool.query(
      `SELECT
    ...
    const { confirmed, total } = anyBookings.rows[0];
    if (Number(total) > 0) {
      const detail = Number(confirmed) > 0
        ? `it has ${confirmed} confirmed and ${total - confirmed} cancelled booking(s)`
        : `it has ${total} cancelled booking(s) on record`;
      return res.status(409).json({
        error: `Cannot delete this event — ${detail}. Events with any booking history cannot be removed, to preserve records.`,
      });
    }

    await pool.query('DELETE FROM events WHERE id = $1', [eventId]);
    res.json({ message: 'Event deleted', eventId: Number(eventId) });
  } catch (err) {
    next(err);
  }
}

module.exports = { createEvent, createShow, listEvents, getEventShows, deleteEvent };
